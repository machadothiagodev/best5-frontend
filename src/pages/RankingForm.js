			...
			<form onSubmit={handleSubmit(submitHandler)}>
				<div className='title'>
					<div className='titlePieces'>
						...
						<select onChange={e => {selectCategoryHandler(e.target.value)}} style={{width: '50%'}}>
							<option value='' />
							{categorias.map(cat => (
						...
						{prodOrServ !== '' && 
							<>						
								<select onChange={e => selectPrepositionHandler(e.target.value)} style={{width: '25%'}}>
									<option value=''></option>
									<option value='DA'>DA</option>
									<option value='DE'>DE</option>
									<option value='DO'>DO</option>
								</select>
								<input type='text' id='region' placeholder='Qual o local do ranking?' {...register('region', { required: true, onChange: event => setRegion(event.target.value) })} />
					...
					{inputItems.map((element, index) => (
						<div key={index} className={`item ${errors[`item${index}`] && 'invalid'}`}>
							...
							<div style={{display: 'flex'}}>
								<input type='text' value={element.name} {...register(`item${index}`, { required: true, onChange: event => changeInputHandler(event, index) })} />
