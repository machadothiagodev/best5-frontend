import { useNavigate, Form, useActionData, useNavigation, Link } from "react-router-dom";

const days = Array.from({ length: 31 }, (e, i) => {
    return i + 1;
});

const months = Array.from({ length: 12 }, (e, i) => {
    return {
        monthName: new Date(null, i + 1, null).toLocaleDateString("pt", { month: "long" }),
        monthNumber: i + 1
    };
});

const currentYear = new Date().getFullYear();
const years = Array(currentYear - (currentYear - 120)).fill('').map((v, idx) => currentYear - idx)

const SignUpForm = () => {

    const actionData = useActionData();
    const navigate = useNavigate();
    const navigation = useNavigation();

    const isSubmitting = navigation.state === 'submitting';

    return (
        <div className="modal_signup">
            <div className="modal_inner">
                <div>
                    <h1>{!(actionData && actionData.confirmEmail) ? 'Cadastrar' : 'Confira seu e-mail'}</h1>
                </div>
                <button type='button' className="close_button" onClick={() => navigate('/rankings')}>
                    <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M8.71146 8.71146C8.79855 8.62415 8.902 8.55489 9.0159 8.50762C9.12979 8.46036 9.2519 8.43604 9.37521 8.43604C9.49852 8.43604 9.62063 8.46036 9.73452 8.50762C9.84842 8.55489 9.95188 8.62415 10.039 8.71146L15.0002 13.6746L19.9615 8.71146C20.0486 8.6243 20.1521 8.55515 20.266 8.50798C20.3799 8.46081 20.5019 8.43653 20.6252 8.43653C20.7485 8.43653 20.8705 8.46081 20.9844 8.50798C21.0983 8.55515 21.2018 8.6243 21.289 8.71146C21.3761 8.79863 21.4453 8.9021 21.4924 9.01599C21.5396 9.12988 21.5639 9.25194 21.5639 9.37521C21.5639 9.49848 21.5396 9.62054 21.4924 9.73443C21.4453 9.84832 21.3761 9.9518 21.289 10.039L16.3258 15.0002L21.289 19.9615C21.3761 20.0486 21.4453 20.1521 21.4924 20.266C21.5396 20.3799 21.5639 20.5019 21.5639 20.6252C21.5639 20.7485 21.5396 20.8705 21.4924 20.9844C21.4453 21.0983 21.3761 21.2018 21.289 21.289C21.2018 21.3761 21.0983 21.4453 20.9844 21.4924C20.8705 21.5396 20.7485 21.5639 20.6252 21.5639C20.5019 21.5639 20.3799 21.5396 20.266 21.4924C20.1521 21.4453 20.0486 21.3761 19.9615 21.289L15.0002 16.3258L10.039 21.289C9.9518 21.3761 9.84832 21.4453 9.73443 21.4924C9.62054 21.5396 9.49848 21.5639 9.37521 21.5639C9.25194 21.5639 9.12988 21.5396 9.01599 21.4924C8.9021 21.4453 8.79863 21.3761 8.71146 21.289C8.6243 21.2018 8.55515 21.0983 8.50798 20.9844C8.46081 20.8705 8.43653 20.7485 8.43653 20.6252C8.43653 20.5019 8.46081 20.3799 8.50798 20.266C8.55515 20.1521 8.6243 20.0486 8.71146 19.9615L13.6746 15.0002L8.71146 10.039C8.62415 9.95188 8.55489 9.84842 8.50762 9.73452C8.46036 9.62063 8.43604 9.49852 8.43604 9.37521C8.43604 9.2519 8.46036 9.12979 8.50762 9.0159C8.55489 8.902 8.62415 8.79855 8.71146 8.71146Z" fill="#282A35"></path>
                    </svg>
                </button>
                {!(actionData && actionData.confirmEmail) &&
                    <Form method='POST'>
                        <div>
                            <div className="email_input_wrapper">
                                <div className="email_input__label_wrapper">
                                    <label htmlFor="firstName">Nome</label>
                                    <span style={{ fontSize: '14px' }}>
                                        Já possui uma conta?
                                        <span className="email_input__label_wrapper_span"><Link to='/login'>Entrar</Link></span>
                                    </span>
                                </div>
                                <div>
                                    <input className="email_input__field_wrapper" type="text" name="firstName" id="firstName" required />
                                </div>
                            </div>
                            <div className="email_input_wrapper">
                                <div className="email_input__label_wrapper">
                                    <label htmlFor="lastName">Sobrenome</label>
                                </div>
                                <div>
                                    <input className="email_input__field_wrapper" type="text" name="lastName" id="lastName" required />
                                </div>
                            </div>
                            <div className="email_input_wrapper">
                                <div className="email_input__label_wrapper">
                                    <label htmlFor="email">Email</label>
                                </div>
                                <div>
                                    <input className="email_input__field_wrapper" type="text" name="email" id="email" required />
                                </div>
                            </div>
                            <div className="email_input_wrapper">
                                <div className="email_input__label_wrapper">
                                    <label htmlFor="birthdayDate">Data de Nascimento</label>
                                </div>
                                <div style={{ display: 'flex' }}>
                                    <select name='day' id='day'>
                                        <option value=''></option>
                                        {days.map(day => <option key={day} value={day}>{day}</option>)}
                                    </select>
                                    <select name='month' id='month' style={{ marginLeft: '12px' }}>
                                        <option value=''></option>
                                        {months.map(month => <option key={month.monthNumber} value={month.monthNumber}>{month.monthName}</option>)}
                                    </select>
                                    <select name='year' id='year' style={{ marginLeft: '12px' }}>
                                        <option value=''></option>
                                        {years.map(year => <option key={year} value={year}>{year}</option>)}
                                    </select>
                                </div>
                            </div>
                            <div className="email_input_wrapper">
                                <div className="email_input__label_wrapper">
                                    <label htmlFor="gender">Gênero</label>
                                </div>
                                <div style={{ display: 'flex' }}>
                                    <span className="gender">
                                        <label className='gender-label'>Masculino</label>
                                        <input type='radio' name='gender' value='Male' />
                                    </span>
                                    <span className="gender" style={{ marginLeft: '12px' }}>
                                        <label className='gender-label'>Feminino</label>
                                        <input type='radio' name='gender' value='Female' />
                                    </span>
                                    <span className="gender" style={{ marginLeft: '12px' }}>
                                        <label className='gender-label'>Outro</label>
                                        <input type='radio' name='gender' value='Other' />
                                    </span>
                                </div>
                            </div>
                            <div className="pass_input_wrapper">
                                <div className="pass_input__label_wrapper">
                                    <label htmlFor="password">Senha</label>
                                </div>
                                <div><input className="pass_input__field_wrapper" name="password" type="password" id="password" required /></div>
                            </div>
                            {actionData && actionData.message && <p style={{ color: 'red' }}>{actionData.message}</p>}
                        </div>
                        <div style={{ height: '20px' }}></div>
                        <div className="bottom_box">
                            <div className="bottom_box_button_login"><button type='submit' disabled={isSubmitting}><span>{isSubmitting ? 'Processando...' : 'Cadastrar'}</span></button></div>
                            {/* <div>
                                <div className="cta_bottom_box_divider"><span className="cta_bottom_box_divider_text">or</span></div>
                                <button type='button' className="button_wrapper_facebook">
                                    <div>
                                        <svg role="img" height="24" width="24" aria-hidden="true" fill="white" viewBox="0 0 24 24" data-encore-id="icon">
                                            <path d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.093 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.563V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z"></path>
                                        </svg>
                                    </div>
                                    <div className="button_wrapper_facebook_text">Continue with Facebook</div>
                                </button>
                                <button type='button' className="button_wrapper_google">
                                    <div>
                                        <svg style={{ color: '#000' }} xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"> <path d="M15.545 6.558a9.42 9.42 0 0 1 .139 1.626c0 2.434-.87 4.492-2.384 5.885h.002C11.978 15.292 10.158 16 8 16A8 8 0 1 1 8 0a7.689 7.689 0 0 1 5.352 2.082l-2.284 2.284A4.347 4.347 0 0 0 8 3.166c-2.087 0-3.86 1.408-4.492 3.304a4.792 4.792 0 0 0 0 3.063h.003c.635 1.893 2.405 3.301 4.492 3.301 1.078 0 2.004-.276 2.722-.764h-.003a3.702 3.702 0 0 0 1.599-2.431H8v-3.08h7.545z" /> </svg>
                                    </div>
                                    <div className="button_wrapper_google_text">Continue with Google</div>
                                </button>
                                <button type='button' className="button_wrapper_google">
                                    <div>
                                        <svg style={{ color: '#000' }} xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"> <path d="M11.182.008C11.148-.03 9.923.023 8.857 1.18c-1.066 1.156-.902 2.482-.878 2.516.024.034 1.52.087 2.475-1.258.955-1.345.762-2.391.728-2.43zm3.314 11.733c-.048-.096-2.325-1.234-2.113-3.422.212-2.189 1.675-2.789 1.698-2.854.023-.065-.597-.79-1.254-1.157a3.692 3.692 0 0 0-1.563-.434c-.108-.003-.483-.095-1.254.116-.508.139-1.653.589-1.968.607-.316.018-1.256-.522-2.267-.665-.647-.125-1.333.131-1.824.328-.49.196-1.422.754-2.074 2.237-.652 1.482-.311 3.83-.067 4.56.244.729.625 1.924 1.273 2.796.576.984 1.34 1.667 1.659 1.899.319.232 1.219.386 1.843.067.502-.308 1.408-.485 1.766-.472.357.013 1.061.154 1.782.539.571.197 1.111.115 1.652-.105.541-.221 1.324-1.059 2.238-2.758.347-.79.505-1.217.473-1.282z" fill="#000000"></path> <path d="M11.182.008C11.148-.03 9.923.023 8.857 1.18c-1.066 1.156-.902 2.482-.878 2.516.024.034 1.52.087 2.475-1.258.955-1.345.762-2.391.728-2.43zm3.314 11.733c-.048-.096-2.325-1.234-2.113-3.422.212-2.189 1.675-2.789 1.698-2.854.023-.065-.597-.79-1.254-1.157a3.692 3.692 0 0 0-1.563-.434c-.108-.003-.483-.095-1.254.116-.508.139-1.653.589-1.968.607-.316.018-1.256-.522-2.267-.665-.647-.125-1.333.131-1.824.328-.49.196-1.422.754-2.074 2.237-.652 1.482-.311 3.83-.067 4.56.244.729.625 1.924 1.273 2.796.576.984 1.34 1.667 1.659 1.899.319.232 1.219.386 1.843.067.502-.308 1.408-.485 1.766-.472.357.013 1.061.154 1.782.539.571.197 1.111.115 1.652-.105.541-.221 1.324-1.059 2.238-2.758.347-.79.505-1.217.473-1.282z" fill="#000000"></path> </svg>
                                    </div>
                                    <div className="button_wrapper_google_text">Continue with Apple</div>
                                </button>
                            </div> */}
                            {/* <div className="forgot_pwd_wrapper"><a href="https://profile.w3schools.com/reset">Forgot password?</a></div> */}
                        </div>
                    </Form>
                }
                {actionData && actionData.confirmEmail &&
                    <Form method='POST'>
                        <input type='hidden' name='formTypeSubmission' value='confirmEmail'></input>
                        <input type='hidden' name='formTypeSubmission' value={actionData.email}></input>
                        <div>
                            <div className="email_input_wrapper">
                                <div className="email_input__label_wrapper">
                                    <label htmlFor="otp">
                                        Enviamos um código de confirmação de quatro dígitos para {actionData.email}. Por favor, insira o código abaixo para confirmar seu endereço de e-mail.
                                    </label>
                                </div>
                                <div>
                                    <input className="email_input__field_wrapper" type="text" name="otp" id="otp" required />
                                </div>
                                <span>Reenviar código</span>
                            </div>
                            {actionData && actionData.message && <p style={{ color: 'red' }}>{actionData.message}</p>}
                        </div>
                        <div style={{ height: '20px' }}></div>
                        <div className="bottom_box">
                            <div className="bottom_box_button_login"><button type='submit' disabled={isSubmitting}><span>{isSubmitting ? 'Processando...' : 'Confirmar'}</span></button></div>
                        </div>
                    </Form>
                }
            </div>
        </div>
    );
}

export default SignUpForm;